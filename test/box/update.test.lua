...
-- Float + float is double when overflow.                                       \
    ...
    {{'float', -flt_max}, {'float', -flt_max}, mp_double},                      \
-- Float + int is float, because no such int exists, which could                \
-- change a max float, even being stored as double.                             \
...
for i, test in pairs(tests) do                                                  \
    ...
    t = t:update({{'+', 1, val2}})                                              \
    t = msgpackffi.encode(t)                                                    \
    if t:byte(1) ~= mp_array_1 or t:byte(2) ~= test[3] then                     \
        err = {i, t:byte(1), t:byte(2)}                                         \
        break                                                                   \
    end                                                                         \
end
err
...
s:replace{1, dbl1, {{key1 = {key2 = {1, dbl1}}}}}
